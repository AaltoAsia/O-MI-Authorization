...
trait AuthRoutes extends JsonSupport {
  ...
  lazy val readRoute = path("read" / Segment) { role_name: String =>
    val permissions: Future[PermissionResult] = authDB.userRulesForRequest(role_name, Read())
    complete(permissions)
  }

  lazy val writeRoute = path("write" / Segment) { role_name: String =>
    val permissions: Future[PermissionResult] = authDB.userRulesForRequest(role_name, Write())
    complete(permissions)
  }
